//program to find the Karl Perarson's Co-efficient of correlation by using Deviation from actual Mean Method.
#include<stdio.h>
#include<stdlib.h>
#include<math.h>
...
int main()
{
	...
	printf("\n");
	//printing actual mean of X and Y 
	for(i=0;i<n;i++)
	{
		sum1 += x[i];
		sum2 += y[i];
	...
	mean1 = (sum1/n);
	mean2 = (sum2/n);
	printf("Assumed Mean of X-series: %d\n",mean1);
	printf("Assumed Mean of Y-series: %d",mean2);
	...
	printf("r = %.4f",r);
	printf("\nTherefore, the Karl Pearson's co-efficient of correlation by using Derivation from Actual Mean method is %.4f.",r);
